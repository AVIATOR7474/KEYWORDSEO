...
def process_document(uploaded_file, primary_keywords, secondary_keywords, h1_max, h2_max, h3_max, 
    ...
    """معالجة المستند مع جميع تحليلات SEO"""
    doc = Document(uploaded_file)
...
def optimize_headings(doc, primary_keywords, secondary_keywords, h1_max, h2_max, h3_max):
    """تحسين هيكل العناوين مع تحديد الحدود"""
    ...
    for para in doc.paragraphs:
        ...
        # التحقق من الكلمات الرئيسية أولاً
        ...
        # إذا لم يتم العثور على كلمة رئيسية، ابحث في الثانوية
        if not used_kw:
        ...
        # إضافة المحتوى مع العناوين المحسنة
        if used_kw and heading_level:
            optimized_doc.add_heading(used_kw, level=heading_level)
            if text != used_kw:  # إضافة النص الأصلي إذا كان مختلفاً عن الكلمة المفتاحية
...
def analyze_content_health(text):
    """تحليل جودة المحتوى لتجربة المستخدم"""
...
def docx_to_html(doc):
    """تحويل DOCX إلى HTML مع الحفاظ على الهيكل"""
    html_content = []
